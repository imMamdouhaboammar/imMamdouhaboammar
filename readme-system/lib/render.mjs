...
const MODEL_BADGES = './assets/profile/model-badges.svg';
...
export function renderPushonomicsBlock({
  ...
  <br />
  <p align="center"><img src="${MODEL_BADGES}" width="410" alt="GPT-5.6 Sol Ultra and Claude Fable 5" /></p>
  <table>
    <tr>
  ...
  <p align="center"><strong>Estimate model:</strong> ${formatNumber(commitsScanned)} commits · ${formatNumber(estimatedInputTokens)} input + ${formatNumber(estimatedOutputTokens)} output · ${escapeHtml(tokensPerChangedLine)} tokens per line · ${escapeHtml(sessionMultiplier)}x session factor · ${formatNumber(mergesExcluded)} merges excluded · estimate, not an invoice.</p>
</details>`;
}
